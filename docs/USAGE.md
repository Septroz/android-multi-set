...
| **Adaptive** | Pass-1 max (same Fixed field, default **64**), min rounds (default **1**), max rounds (default **18**), absolute cap (default **262144**, hard max **2^30**) | Full progressive pass at pass-1 max, then only re-tests interior border pixels (4-connected to an escaped neighbor). Minimum rounds keeps doubling after an empty pass, up to the configured number of rounds; this can reveal detail that needs more iterations. Intermediate fills are painted on screen. The status overlay **Iter** shows the max from the last border round; zoom does not raise pass-1 from that value. All-black progressive previews are skipped until the first border escape. |
...
proportion to completed samples from a coarse power-of-two grid down to
individual pixels, weighted by work. In **Adaptive** mode, after the
individual-pixel pass the bar switches to an indeterminate
